...
cp "$BUILD_DIR/audiodecoder.openmpt.so.21.0.2" "$BUILD_DIR/audiodecoder.openmpt/"
...
if [ -n "$LIBMPG123" ]; then
    ...
    if command -v patchelf &> /dev/null; then
        patchelf --set-rpath '$ORIGIN' "$BUILD_DIR/audiodecoder.openmpt/audiodecoder.openmpt.so.21.0.2"
